...
function CustomTaskNode({ data }: NodeProps) {
  ...
  return (
    <div
      className={`w-56 rounded-lg border-2 p-2.5 shadow-xl transition-all hover:scale-[1.02] cursor-pointer ${getStatusColor(
        data.status as TaskStatus
      )} ${Boolean(data.isCritical) ? 'ring-2 ring-pink-500 shadow-pink-500/20' : ''}`}
    >
      <Handle
        type="target"
        position={Position.Left}
        className="w-2.5 h-2.5 bg-indigo-500 border-2 border-zinc-900 rounded-full"
      />

      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-[9px]">
          <span className="font-bold uppercase tracking-wider px-1 py-0.2 rounded bg-zinc-950/80 border border-zinc-800">
            {data.priority as string}
          </span>
          {Boolean(data.isCritical) && (
            <span className="font-bold px-1 py-0.2 rounded bg-pink-500/20 text-pink-400 border border-pink-500/40 animate-pulse">
              CRITICAL
            </span>
          )}
          <span className="font-mono px-1 py-0.2 rounded bg-zinc-950 text-zinc-400">
        ...
        <div className="flex items-center justify-between text-[10px] text-zinc-400 pt-1 border-t border-zinc-800/80">
          <span className="flex items-center gap-1 font-medium">
            {data.worker ? (
              <>
                ...
                <span className="truncate max-w-[70px]">{(data.worker as any).name}</span>
              </>
            ) : (
          ...
          {Boolean(data.estimatedDuration) ? (
            <span className="flex items-center gap-0.5 font-mono text-[9px]">
              <Clock className="w-2.5 h-2.5" /> {String(data.estimatedDuration)}m
            </span>
          ) : null}
        </div>
      </div>
      ...
      <Handle
        ...
        className="w-2.5 h-2.5 bg-emerald-500 border-2 border-zinc-900 rounded-full"
      />
    </div>
  );
}
...
export default function DependencyGraph({ graphData, onOpenTask }: DependencyGraphProps) {
  ...
  return (
    <div className="relative w-full h-full rounded-xl bg-zinc-950 border border-zinc-800 overflow-hidden shadow-2xl min-h-0 flex-1">
      {/* Top Legend */}
      <div className="absolute top-2 left-2 z-10 bg-zinc-900/90 backdrop-blur-md border border-zinc-800 rounded-lg px-2.5 py-1.5 shadow-xl flex items-center gap-3 text-[10px]">
        ...
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 rounded-full bg-pink-500 ring-2 ring-pink-500/40" />
